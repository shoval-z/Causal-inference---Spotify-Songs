...
def load_data_after_trimming(df, t1):
    ...
    :return: X- the scale features.
            T_dance - the column that represent the treatment group
    ...
    scaler = MinMaxScaler()
    T_dance_name = ['danceability_0', 'danceability_1', 'danceability_2',
                    'danceability_3', 'danceability_4', 'danceability_5',
                    'danceability_6', 'danceability_7', 'danceability_8',
                    'danceability_9']
    X = df.drop(T_dance_name, axis=1)
    X = scaler.fit_transform(X.drop(['song_popularity'], axis=1))
    T_dance = df[t1]
    Y = df['song_popularity']
    return X, T_dance, Y, df
...
def load_data(CATE=False):
    ...
    Q_5 = df.quantile(0.01, axis=0)['danceability']
    Q_95 = df.quantile(0.99, axis=0)['danceability']
    df = df[(df['danceability'] >= Q_5) & (df['danceability'] <= Q_95)]
    _, deci = pd.qcut(df['danceability'], 10, retbins=True)
    df['delta_danceability'] = df.apply(delta_danceability, args=tuple([deci]), axis=1)
    categorial = ['artist_name', 'playlist']
    original_df = df.copy()
    df = df.join(pd.get_dummies(df[categorial]))
    df = df.join(pd.get_dummies(df.delta_danceability, prefix='danceability'))
    original_df = original_df.join(pd.get_dummies(df.delta_danceability, prefix='danceability'))

    df = df.drop(
        ['song_name', 'song_namesong_name', 'album_names', 'artist_name', 'playlist', 'key', 'delta_danceability',
         'danceability'], axis=1)
    original_df = original_df.drop(
        ['song_name', 'song_namesong_name', 'album_names', 'key', 'danceability', 'delta_danceability'], axis=1)
    scaler = MinMaxScaler()
    T_dance_name = ['danceability_0', 'danceability_1', 'danceability_2',
                    'danceability_3', 'danceability_4', 'danceability_5',
                    'danceability_6', 'danceability_7', 'danceability_8',
    ...
    T_dance = df[T_dance_name]
    Y = df['song_popularity']

    return X, T_dance, Y, df, original_df
...
def delta_danceability(x, deci):
    """
    define the new delta columns according to the deciles of the dancebility columns
    :param x: a sample
    :param deci: list od the deciles
    :return: the correct values for the given sample
    """
    if x['danceability'] < deci[1]:
        return 0
    elif x['danceability'] < deci[2]:
        return 1
    elif x['danceability'] < deci[3]:
        return 2
    elif x['danceability'] < deci[4]:
        return 3
    elif x['danceability'] < deci[5]:
        return 4
    elif x['danceability'] < deci[6]:
        return 5
    elif x['danceability'] < deci[7]:
        return 6
    elif x['danceability'] < deci[8]:
        return 7
    elif x['danceability'] < deci[9]:
        return 8
    else:
        return 9
...
def main():
    ...
    """
    CATE = True
    X, T_dance, Y, df, original_df = load_data(CATE=CATE)
    CATE_dance_list = [('danceability_1', 'danceability_2'), ('danceability_2', 'danceability_3'),
                       ('danceability_4', 'danceability_5'), ('danceability_5', 'danceability_6'),
                       ('danceability_6', 'danceability_7'), ('danceability_7', 'danceability_8'),
                       ('danceability_8', 'danceability_9')]
    ATE_dict_s = {'lasso': dict(), 'ridge': dict()}
    ATE_dict_t = {'lasso': dict(), 'ridge': dict()}
    ATE_ipw = dict()

    for i, t1 in enumerate(T_dance.columns):
        for j, t2 in enumerate(T_dance.columns):
            if j != i + 1:
                continue
            if CATE:
                if (t1, t2) not in CATE_dance_list:
                    continue
            new_df, unwanted_df, wanted_pair, lr_model = propensity_score(X, T_dance, original_df, t1, t2)
            if not wanted_pair:
                continue
    ...
    np.save(f'CI_S_learner_LASSO_dance_{CATE}.npy', s_lasso_ci)
    np.save(f'CI_S_learner_RIDGE_dance_{CATE}.npy', s_ridge_ci)
    np.save(f'CI_T_learner_LASSO_dance_{CATE}.npy', t_lasso_ci)
    np.save(f'CI_T_learner_RIDGE_dance_{CATE}.npy', t_ridge_ci)
    np.save(f'CI_IPW_dance_{CATE}.npy', ipw_ci)
